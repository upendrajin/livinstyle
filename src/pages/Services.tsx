      ...
      <div className="pt-32 pb-24 bg-background">
        <div className="container mx-auto px-4 lg:px-8">
            ...
            {categories.map((c) => (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                  active === c ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground/60 hover:bg-secondary/80"
                }`}
              >
                {c}
              </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filtered.map((s, i) => (
              <div key={i} className="bg-card rounded-xl overflow-hidden shadow-card group hover:shadow-elegant transition-all duration-400">
